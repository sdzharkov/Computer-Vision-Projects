...
corners(:,1) = [1;1;1]; % treating these coordinates as (x,y,1) 
...
reshaped = H \ reshaped; % apply the inv(H) onto the reshaped matrix

for k=1:width*height % Normalize all the values with each W
    reshaped(:,k) = reshaped(:,k) ./ reshaped(3,k);
end

reshaped(3,:) = []; % now we have the warped coordinates 
xVal = round(reshaped(1,:))'; % x values of the new coordinates
yVal = round(reshaped(2,:))'; % y values of the new coordinates
